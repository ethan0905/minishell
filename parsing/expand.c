...
void	join_expand(t_data *data, char **final_str, char **str, \
...
char	*expand_str(t_data *data, char *str)
{
	...
	{
		expand_exec = false;
		quoting_gest(*str, &is_in_single_quote, &is_in_double_quote);
		if (*str == '$' && (*(str + 1) && (*(str + 1) != '\'' && \
		*(str + 1) != '\"')) && !is_in_single_quote)
		{
			expand_exec = true;
